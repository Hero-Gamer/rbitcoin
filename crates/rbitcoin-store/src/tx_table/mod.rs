...
impl TxTable {
    ...
    /// BIP158 basic filter elements of one block's creates `first..=last`.
    ///
    /// Every output script except OP_RETURN, then the script of every prevout
    /// its inputs spend (parent edges from `input.body`; no `tx.head` probe and
    /// no `seqsigwit`, so pruned heights still build). Unordered and may
    /// repeat: the filter writer dedupes by bytes.
    pub fn basic_filter_elements(&self, first: u64, last: u64) -> Result<Vec<Vec<u8>>, StoreError> {
        ...
        let locs = self
            ...
            .into_iter()
            .map(|p| {
                p.ok_or(StoreError::Corrupt(
                    "invariant: blockfilter create loc missing",
                ))
            })
            .collect::<Result<Vec<_>, _>>()?;
        let lo = locs.iter().map(|p| p.txout.0).min().unwrap_or(0);
        let hi = locs
            .iter()
            .map(|p| p.txout.0.saturating_add(p.txout.1))
            .max()
            .unwrap_or(0);
        let mut out = Vec::new();
        self.body.with_bytes_at_pread(lo, hi - lo, |buf| {
            for p in &locs {
                let rel = (p.txout.0 - lo) as usize;
                let raw = buf
                    .get(rel..rel + p.txout.1 as usize)
                    .ok_or(StoreError::Corrupt("txout span short for fk range"))?;
                visit_packed_scripts(raw, p.n_out, Some(&self.secret), |script| {
        ...
        let mut parents: std::collections::BTreeMap<u64, Vec<u32>> = Default::default();
        for edges in self.input.edges_span(first, last)? {
            let edges = edges.ok_or(StoreError::Corrupt(
                "invariant: blockfilter input edges unstamped",
            ))?;
            for e in edges.into_iter().filter(|e| !e.parent.is_null()) {
                parents.entry(e.parent.0).or_default().push(e.vout);
            }
        }
        let parent_fks: Vec<Fk> = parents.keys().map(|&fk| Fk(fk)).collect();
        let parent_locs = self.create_loc.range_batch(&parent_fks)?;
        let mut jobs: Vec<OutsByRangeJob> = Vec::with_capacity(parent_fks.len());
        for ((fk, mut vouts), loc) in parents.into_iter().zip(parent_locs) {
            let fk = Fk(fk);
            let loc = loc.ok_or(StoreError::Corrupt(
                "invariant: blockfilter prevout loc missing",
            ))?;
            vouts.sort_unstable();
            vouts.dedup();
            jobs.push((fk, loc.txout, [0u8; 32], loc.n_out, vouts));
        }
        for (row, job) in self
            .get_outs_by_range_batch(&jobs)?
            .0
            .into_iter()
            .zip(&jobs)
        {
            let (_, live, _) = row.ok_or(StoreError::Corrupt(
                "invariant: blockfilter prevout missing",
            ))?;
            if live.len() != job.4.len() {
                return Err(StoreError::Corrupt(
                    "invariant: blockfilter prevout missing",
                ));
            }
            out.extend(live.into_iter().map(|(_, o)| o.script));
        }
        Ok(out)
    }
