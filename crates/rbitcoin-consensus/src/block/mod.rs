...
mod script_detection_kills {
    ...
    use bitcoin::Block;
    use bitcoin::{Amount, BlockHash, CompactTarget, TxMerkleNode};
    use rbitcoin_primitives::Height;

    // ================================================================
    // GROUP A — Script Type Detection
    // ================================================================

    #[test]
    fn kill_is_p2sh_script_428() {
        let p2sh = &[
            0xa9, 0x14, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
            0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x87,
        ];
        assert!(is_p2sh_script(p2sh), "valid P2SH → true");
        let p2pkh = &[
            0x76, 0xa9, 0x14, 0xab, 0xcd, 0xef, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x88, 0xac,
        ];
        assert!(!is_p2sh_script(p2pkh), "P2PKH pattern → false");
        let wrong_first = &[
            0xaa, 0x14, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
            0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x87,
        ];
        assert!(!is_p2sh_script(wrong_first), "first byte 0xaa → false");
        let wrong_last = &[
            0xa9, 0x14, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
            0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x99,
        ];
        assert!(!is_p2sh_script(wrong_last), "last byte 0x99 → false");
        assert!(!is_p2sh_script(&[]), "empty → false");
    }

    #[test]
    fn kill_is_p2wpkh_program_433() {
        let p2wpkh = &[
            0x00, 0x14, 0x75, 0x1e, 0x76, 0xe8, 0x19, 0x99, 0x6d, 0x45, 0x2d, 0x2d, 0x90, 0xe7,
            0x33, 0xfa, 0x99, 0x8d, 0x79, 0x30, 0x0d, 0x22,
        ];
        assert_eq!(p2wpkh.len(), 22);
        assert!(is_p2wpkh_program(p2wpkh), "valid P2WPKH → true");
        let p2wsh_len = &[
            0x00, 0x20, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
            0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19,
            0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f,
        ];
        assert!(!is_p2wpkh_program(p2wsh_len), "P2WSH length → false");
        let v1 = &[
            0x51, 0x14, 0x75, 0x1e, 0x76, 0xe8, 0x19, 0x99, 0x6d, 0x45, 0x2d, 0x2d, 0x90, 0xe7,
            0x33, 0xfa, 0x99, 0x8d, 0x79, 0x30, 0x0d, 0x22,
        ];
        assert!(!is_p2wpkh_program(v1), "v0=0x51 → false");
        assert!(!is_p2wpkh_program(&[]), "empty → false");
    }

    #[test]
    fn kill_is_p2wsh_program_438() {
        let p2wsh = &[
            0x00, 0x20, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
            0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19,
            0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f,
        ];
        assert_eq!(p2wsh.len(), 34);
        assert!(is_p2wsh_program(p2wsh), "valid P2WSH → true");
        let p2wpkh_len = &[
            0x00, 0x14, 0x75, 0x1e, 0x76, 0xe8, 0x19, 0x99, 0x6d, 0x45, 0x2d, 0x2d, 0x90, 0xe7,
            0x33, 0xfa, 0x99, 0x8d, 0x79, 0x30, 0x0d, 0x22,
        ];
        assert!(!is_p2wsh_program(p2wpkh_len), "P2WPKH length → false");
        let v1 = &[
            0x51, 0x20, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
            0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19,
            0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f,
        ];
        assert!(!is_p2wsh_program(v1), "v0=0x51 → false");
        assert!(!is_p2wsh_program(&[]), "empty → false");
    }

    // ================================================================
    // GROUP B — Witness Malleation: FULL BRANCH COVERAGE
    // ================================================================

    #[test]
    fn kill_witness_commitment_293() {
        const MAGIC: [u8; 6] = [0x6a, 0x24, 0xaa, 0x21, 0xa9, 0xed];
        let mut valid = [0u8; 38];
        valid[..6].copy_from_slice(&MAGIC);
        assert!(witness_commitment_vout_index(&make_test_tx(&valid)).is_some());
    }

    #[test]
    fn kill_witness_malleation_263() {
        // ┌─────────────────────────────────────────────────────┐
        // │ PUBLIC API USED: apply_test_activation_height       │
        // │ Sets segwit_height_overlay = 10 → clean pre/post split│
        // └─────────────────────────────────────────────────────┘

        const SEGWIT_ACTIVATION: u32 = 10;
        let mut params = ChainParams::regtest();
        // Official test API — equivalent to `-testactivationheight=segwit@10`
        params
            .apply_test_activation_height("segwit", SEGWIT_ACTIVATION)
            .expect("Failed to set segwit activation height");

        // Sanity: confirm our setup actually changes behavior
        assert!(
            params.segwit_active_at(SEGWIT_ACTIVATION),
            "setup: active at boundary"
        );
        assert!(
            !params.segwit_active_at(SEGWIT_ACTIVATION - 1),
            "setup: inactive just before"
        );
        assert!(!params.segwit_active_at(0), "setup: genesis pre-activation");

        let height_pre = Height(SEGWIT_ACTIVATION - 1); // Pre-activation
        let height_post = Height(SEGWIT_ACTIVATION); // Active

        let ctx_pre = ValidationContext::at(&params, height_pre, Milestone::default());
        let ctx_post = ValidationContext::at(&params, height_post, Milestone::default());
        let mut ctx_pre_no_gate = ValidationContext::at(&params, height_pre, Milestone::default());
        ctx_pre_no_gate.enforce_height_gates = false;

        // --- Commitment data ---
        let reserved: [u8; 32] = [0xAB; 32];
        let witness_root = [0u8; 32];
        let mut hash_input = [0u8; 64];
        hash_input[..32].copy_from_slice(&witness_root);
        hash_input[32..].copy_from_slice(&reserved);
        let commit_hash = bitcoin_hashes::sha256d::Hash::hash(&hash_input);
        let commit_bytes = commit_hash.to_byte_array();

        let mut commitment_script = [0u8; 38];
        commitment_script[0] = 0x6a;
        commitment_script[1] = 0x24;
        commitment_script[2..6].copy_from_slice(&[0xaa, 0x21, 0xa9, 0xed]);
        commitment_script[6..38].copy_from_slice(&commit_bytes);

        let zero32 = [0u8; 32];
        let header = Header {
            version: bitcoin::block::Version::ONE,
            prev_blockhash: BlockHash::from_byte_array(zero32),
            merkle_root: TxMerkleNode::from_byte_array(zero32),
            time: 0,
            bits: CompactTarget::from_consensus(0x1d00ffff),
            nonce: 0,
        };

        // ── Block A: WITH witness + commitment ──
        let tx_with_witness = Transaction {
            ...
            output: vec![
                ...
                TxOut {
                    value: Amount::ZERO,
                    script_pubkey: ScriptBuf::from_bytes(commitment_script.to_vec()),
                },
            ],
        };
        ...
        tx_with_witness.consensus_encode(&mut wire).unwrap();
        let pres_with_witness = vec![rbitcoin_query::TxPrecompute::from_tx_wire(
            &tx_with_witness,
            &wire,
            true,
        )];
        assert!(
            pres_with_witness[0].has_witness,
            "Witness detection must be true"
        );
        let block_with_witness = Block {
            header,
            txdata: vec![tx_with_witness],
        };
        ...
        let pres_no_witness = vec![rbitcoin_query::TxPrecompute::from_tx(&tx_no_witness)];
        assert!(!pres_no_witness[0].has_witness, "Clean precompute expected");
        let block_no_witness = Block {
            header,
            txdata: vec![tx_no_witness],
        };

        // ── Block C: commitment ONLY (no witness in tx) ──
        let tx_commit_only = Transaction {
            version: bitcoin::transaction::Version::ONE,
            lock_time: LockTime::ZERO,
            input: vec![TxIn {
                previous_output: OutPoint::default(),
        ...
        let pres_commit_only = vec![rbitcoin_query::TxPrecompute::from_tx(&tx_commit_only)];
        assert!(
            !pres_commit_only[0].has_witness,
            "Commitment-only has no witness"
        );
        let block_commit_only = Block {
            header,
            txdata: vec![tx_commit_only],
        };

        // ┌─────────────────────────────────────────────────────┐
        // │ 5 ASSERTIONS — EVERY BRANCH EXERCISED                │
        // ├───────────┬──────────┬──────────┬───────────────────┤
        // │ Scenario │ Height   │ Gates    │ Expected           │
        // ├───────────┼──────────┼──────────┼───────────────────┤
        // │ Witness   │ Pre(9)   │ On       │ REJECT (line 263)  │
        // │ Witness   │ Post(10) │ On       │ PASS (check commit)│
        // │ Witness   │ Pre(9)   │ Off      │ PASS (bypass)      │
        // │ No-wit    │ Pre(9)   │ On       │ PASS (no trigger)  │
        // │ Commit-   │ Pre(9)   │ On       │ PASS (line 266)    │
        // │ only      │          │          │                     │
        // └───────────┴──────────┴──────────┴───────────────────┘
        ...
        // 2. POST-SEGWIT + WITNESS → PASS
        let r2 = reject_witness_malleation(&block_with_witness, &ctx_post, &pres_with_witness);
        assert!(r2.is_ok(), "2. Post-segwit+witness MUST PASS; got {:?}", r2);

        // 3. PRE-SEGWIT + BYPASS → PASS
        let r3 =
            reject_witness_malleation(&block_with_witness, &ctx_pre_no_gate, &pres_with_witness);
        assert!(r3.is_ok(), "3. Pre-segwit+no-gate MUST PASS; got {:?}", r3);

        // 4. PRE-SEGWIT + NO WITNESS → PASS
        let r4 = reject_witness_malleation(&block_no_witness, &ctx_pre, &pres_no_witness);
        assert!(
            r4.is_ok(),
            "4. Pre-segwit+no-witness MUST PASS; got {:?}",
            r4
        );

        // 5. PRE-SEGWIT + COMMITMENT ONLY → PASS
        let r5 = reject_witness_malleation(&block_commit_only, &ctx_pre, &pres_commit_only);
        assert!(
            r5.is_ok(),
            "5. Pre-segwit+commitment-only MUST PASS; got {:?}",
            r5
        );
    }

    fn make_test_tx(script: &[u8]) -> Transaction {
        Transaction {
            version: bitcoin::transaction::Version::ONE,
            lock_time: LockTime::ZERO,
            input: vec![],
    ...
    // ================================================================
    // GROUP C — Boundary Mutant Killers
    // ================================================================
    #[test]
    fn kill_boundary_240() {
        // Boundary: segwit activation height — just before, exactly, just after
        let mut params = ChainParams::regtest();
        params.apply_test_activation_height("segwit", 100).unwrap();

        // h = activation - 1 → PRE-activation
        let ctx_pre = ValidationContext::at(&params, Height(99), Milestone::default());
        let pre_active = ctx_pre.params.segwit_active_at(ctx_pre.height.0);
        assert!(!pre_active, "h=99 must be PRE-activation");

        // h = activation → ACTIVE
        let ctx_at = ValidationContext::at(&params, Height(100), Milestone::default());
        let at_active = ctx_at.params.segwit_active_at(ctx_at.height.0);
        assert!(at_active, "h=100 must be ACTIVE");

        // h = activation + 1 → ACTIVE
        let ctx_post = ValidationContext::at(&params, Height(101), Milestone::default());
        let post_active = ctx_post.params.segwit_active_at(ctx_post.height.0);
        assert!(post_active, "h=101 must be ACTIVE");
    }

    #[test]
    fn kill_boundary_187() {
        // Boundary: genesis/minimal height edge cases (h=0, pre/post)
        let mut params = ChainParams::regtest();
        params.apply_test_activation_height("segwit", 5).unwrap();

        // h = 0 — genesis
        let ctx_genesis = ValidationContext::at(&params, Height(0), Milestone::default());
        let h0_active = ctx_genesis.params.segwit_active_at(ctx_genesis.height.0);
        assert!(!h0_active, "h=0 must be pre-activation");

        // h = activation - 1
        let ctx_pre = ValidationContext::at(&params, Height(4), Milestone::default());
        let h4_active = ctx_pre.params.segwit_active_at(ctx_pre.height.0);
        assert!(!h4_active, "h=4 must be pre-activation");

        // h = activation
        let ctx_at = ValidationContext::at(&params, Height(5), Milestone::default());
        let h5_active = ctx_at.params.segwit_active_at(ctx_at.height.0);
        assert!(h5_active, "h=5 must be active");
    }

    #[test]
    fn kill_boundary_74() {
        // Boundary: enforce_height_gates — flag does NOT affect active_at
        let mut params = ChainParams::regtest();
        params.apply_test_activation_height("segwit", 50).unwrap();

        // Same height, gate ON
        let mut ctx_gate_on = ValidationContext::at(&params, Height(25), Milestone::default());
        ctx_gate_on.enforce_height_gates = true;
        let active_gate_on = ctx_gate_on.params.segwit_active_at(ctx_gate_on.height.0);

        // Same height, gate OFF
        let mut ctx_gate_off = ValidationContext::at(&params, Height(25), Milestone::default());
        ctx_gate_off.enforce_height_gates = false;
        let active_gate_off = ctx_gate_off.params.segwit_active_at(ctx_gate_off.height.0);

        // active_at is pure — must NOT depend on gate flag
        assert_eq!(
            active_gate_on, active_gate_off,
            "segwit_active_at must be independent of enforce_height_gates"
        );
        assert!(
            !active_gate_on,
            "h=25 < 50 must be pre-activation regardless of gate"
        );
    }
