...
mod pr3_kills_56 {
    use super::*;
    use crate::convert::header_to_record;
    ...
    fn find(prev: BlockHash, time: u32, bits: CompactTarget) -> Header {
        for nonce in 0..200_000u32 {
            ...
            if Target::from_compact(bits)
                .is_met_by(BlockHash::from_byte_array(h.block_hash().to_byte_array()))
            {
                return h;
            }
        }
    ...
    fn kills_56() {
        ...
        let genesis = find(BlockHash::from_byte_array([0; 32]), 1231006505, bits);
        let gh = genesis.block_hash().to_byte_array();
        let rec = header_to_record(Fk::NULL, &genesis, gh);
        let fk = q.put_header(&rec).unwrap();
        // THIS fixes tip=None -> Some(0) so header_at_height(0) works
        q.store().confirmed.set(Height(0), fk).unwrap();
        assert_eq!(q.tip_height(), Some(Height(0)));

        let h1_ok = find(BlockHash::from_byte_array(gh), 1231006506, bits);
        let h1_ok_hash = h1_ok.block_hash().to_byte_array();
        let mut params = ChainParams::regtest();
        params.checkpoints = vec![Checkpoint {
            height: 1,
            hash: BlockHash::from_byte_array(h1_ok_hash),
        }];

        let h1_bad = find(BlockHash::from_byte_array(gh), 1231006506, bits);
        let bad_hash = if h1_bad.block_hash().to_byte_array() == h1_ok_hash {
            [2u8; 32]
        } else {
            h1_bad.block_hash().to_byte_array()
        };

        let rb = validate_header_hashed(&q, &params, Height(1), &h1_bad, bad_hash);
        assert!(rb.is_err(), "bad checkpoint must Err");

        let ro = validate_header_hashed(&q, &params, Height(1), &h1_ok, h1_ok_hash);
        println!("res_ok={:?}", ro);
        assert!(ro.is_ok(), "good checkpoint must Ok");
...
mod pr3_kills_78_82 {
    ...
    use rbitcoin_primitives::Height;
    #[test]
    fn kills_78_time_eq_mtp() {
        let params = ChainParams::regtest();
        let bits = CompactTarget::from_consensus(0x207fffff);
        let parent_mtp = 1000u32;
        ...
        // time == mtp : original <= => Err, mutant > => Ok, so Err kills <=->>
        let res_eq = validate_header_on_parent(&params, Height(1), &header, parent_mtp, bits);
        assert!(res_eq.is_err(), "time==mtp must Err (kills 78)");

        header.time = parent_mtp + 1;
        // need valid pow for this time, find nonce
        for nonce in 0..200_000u32 {
            header.nonce = nonce;
            if validate_header_on_parent(&params, Height(1), &header, parent_mtp, bits).is_ok() {
                break;
            }
        }
        let res_gt = validate_header_on_parent(&params, Height(1), &header, parent_mtp, bits);
        // may still fail pow but not mtp — we just check mtp part passed (not BadHeader mtp)
        // so we assert not mtp error by checking time > mtp passes mtp gate
        // pow may still fail, but we only care mtp gate was passed
        // we test with easy bits so pow passes quickly
        println!("res_gt={:?}", res_gt);
    }
    #[test]
    fn kills_82_bits_mismatch() {
        let params = ChainParams::regtest();
        let bits_ok = CompactTarget::from_consensus(0x207fffff);
        let bits_bad = CompactTarget::from_consensus(0x1d00ffff);
        let parent_mtp = 0u32;
        let mut header = Header {
            version: Version::from_consensus(4),
            prev_blockhash: BlockHash::from_byte_array([0u8; 32]),
        ...
        for nonce in 0..200_000u32 {
            header.nonce = nonce;
            if validate_header_on_parent(&params, Height(1), &header, parent_mtp, bits_ok).is_ok() {
                break;
            }
        }
        assert!(
            validate_header_on_parent(&params, Height(1), &header, parent_mtp, bits_ok).is_ok()
        );

        header.bits = bits_bad;
        let res_bad = validate_header_on_parent(&params, Height(1), &header, parent_mtp, bits_ok);
        assert!(res_bad.is_err(), "bits mismatch must Err (kills 82)");
    }
}
