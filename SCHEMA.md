...
<datadir>/
  store/
    ...
    blockfilters/basic.headers        # optional BIP158 basic: 32 B header per height (`--block-filter-index`)
    blockfilters/basic.bodies         # u32 LE length + filter bytes, same height order. Missing files: index off
