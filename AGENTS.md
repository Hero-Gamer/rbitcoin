...
- PR `mutants (0/4)` through `mutants (3/4)` are required. A 30-minute
  kill with no `MISSED` warns and passes. `MISSED` before that kill fails
  the shard. A finished non-zero `cargo mutants` exit fails. Owner:
