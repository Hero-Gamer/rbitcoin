...
| `core_analogs::analog_block_filters_from_class_a` | Store+query | BIP158 basic filter built from Class A (`input.body` parent edges + `txout`) equals rust-bitcoin `new_script_filter` at every height: coinbase-only, a spend, and a block with duplicate scripts, OP_RETURN, and a segwit output. After `--prune-seqsigwit` passes that block, reconstruct refuses and the build still matches. `rbtc-bf-wb` materializes the whole chain in parallel chunks, stopped after its first commit and restarted: every stored filter equals the serial build and the header chain is unbroken. A tip reorg while the index is off: reopening with it on drops the stale-branch slot (`header_fk` ≠ `confirmed[h]`) and the rebuilt filter matches the new block (`rpc_getblockfilter.py`, `feature_blockfilterindex_prune.py`) |
...
| `tip_follow_after_ibd` | P2P (**default**) | After IBD, follow + one new tip via inv/headers. With the filter index on, IBD confirm writes no basic filters; `rbtc-bf-wb` materializes them to the tip, then seals the followed block |
