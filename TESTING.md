...
| `core_analogs::analog_block_filters_from_class_a` | Store+query | BIP158 basic filter built from Class A (`input.body` parent edges + `txout`) equals rust-bitcoin `new_script_filter` at every height: coinbase-only, a spend, and a block with duplicate scripts, OP_RETURN, and a segwit output. After `--prune-seqsigwit` passes that block, reconstruct refuses and the build still matches. A tip reorg while the index is off: reopening with it on drops the stale-branch slot (`header_fk` ≠ `confirmed[h]`) and the rebuilt filter matches the new block (`rpc_getblockfilter.py`, `feature_blockfilterindex_prune.py`) |
