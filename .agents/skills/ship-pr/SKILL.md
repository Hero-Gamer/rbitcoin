...
`macos`, `coverage`, `nixos-module-eval`, `scripts`, and `mutants (0/4)`
through `mutants (3/4)`. A mutants shard killed at 30 minutes with no
`MISSED` warns and passes. `MISSED` before that kill fails the shard. A
