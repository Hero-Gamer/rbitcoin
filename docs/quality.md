...
| **—** | Chained Esplora `scripthash_mempool_stats` | Dialect / page cost. `/blocks` size/weight is persisted on `header.body` (schema 24). Graphical explorer APIs stay Won't-fix. |
...
| **Q-59** | RPC / CLI honesty | `submitblock` all-networks via `ChainHub`. `--minrelaytxfee` garbage/negatives fail start. `getnetworkhashps` labeled dummy 2-work-per-block. `gettxout include_mempool` hides mempool-spent confirmed outs. RPC-submit `maxfeerate` (sat/vB, default 10000, `0` unlimited, `>=100000` is `-8`) / `maxburnamount` (default 0) on sendraw / testmempoolaccept / submitpackage; P2P `accept_tx` uncapped. `--rpcworkqueue` full permit is HTTP 503 (one POST is one slot). `getmininginfo.blockmintxfee` is `sat_btc_json` BTC/kvB. |
