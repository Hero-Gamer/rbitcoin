...
| `estimatesmartfee` | **10-minute inclusion frontier** — not Core historical multi-horizon. Core's result shape: `{feerate, blocks}`, or `{errors, blocks}` with no `feerate` when there is no estimate. See [`mempool-fee-estimation.md`](./mempool-fee-estimation.md). |
