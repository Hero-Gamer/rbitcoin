...
**Packages:** RPC `submitpackage` is sequential `accept_tx`. Esplora
`POST /txs/package` is atomic `accept_package`. No P2P package command. BIP331
`NetworkMessage` needs a rust-bitcoin upgrade.
...
| Risk | Notes |
|------|--------|
| Disk / RAM | Multi‑100 GiB Class A; segmented 25-bit `tx.head.*` + mapped `.fuse8` (~1.5 GiB `RssFile`, heap `fuse8=0`); sealed BDZ `g` FdOnly (not anon heap) |
