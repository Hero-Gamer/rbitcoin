...
| `--block-filter-index` | `block_filter_index=` | **off** — BIP158 basic filters. Independent of `--sh-index`. IBD does not build them. After catch-up the `rbtc-bf-wb` appender materializes the gap from Class A in the background (`blockfilter: materialize from=… to=…`, progress every 10 s, `… done`), then seals each new tip (`blockfilter: apply h=…`); follow, relay, and Electrum do not wait for it. Works with `--prune-seqsigwit`. `tip: accept` shows `bf=` and `bf_lag=`. `NODE_COMPACT_FILTERS` is advertised once filters first reach the tip (`blockfilter: caught up …`); peers that connected earlier do not learn the bit. `getblockfilter`, `/rest/blockfilter/`, and P2P serve heights the watermark covers; a stop past the watermark is silence |
...
| `--sp-tweaks` | `sp_tweaks=` | **off** — thin BIP-352 tweak index (`sp_tweaks.*`). Refused with `--prune-seqsigwit`: tweaks read input keys from scriptSig and witness |
