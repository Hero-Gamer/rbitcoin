...
4. Replay spend annotations for every height in `(A, tip]` (`spend_durable` annotated-through). A missing file checks the last 6 heights. When those spends already match, publish the marker at the tip and rewrite nothing. A mismatch sets `A = 0` and replays the chain. `finish_post_commit_hashes` is idempotent. Then `sync_data` the replay inputs and publish `A` and durable-through `D` at the tip. The replay logs height progress every 10 seconds.
