...
| Confirm **load** | 1 OS thread | `confirm_wire_lookup_stamp` (structure with **loadq `pres`**, plan_batch binds carried BQ keys, skeleton bind) + pin `txout` + assemble |
| Confirm **scripts** | 1 OS thread (`ibd-confirm`) publishes waves + `rbtc-scripts` steal (lock-free claim) | **none** — pure CPU |
| Confirm **write** | 1 OS thread (`ibd-confirm-write`) + 1 process-wide `ibd-confirm-head` | **sole Class A appender** (`txout`+`seqsigwit`+`spent`; IBD encodes ins from `Arc<Block>` + SpendEdges) + structural + Class C + spend annotate on **`spent.body`** + tip GC; **`block_queue_dequeue_height`**. `tx.head` write-behind insert runs on **`ibd-confirm-head`** overlapping structural + Class C (not a per-batch spawn). Class A **never leads tip** (same commit era; no archive-ahead DONTNEED) |
...
**Height-ordered unified pipeline (current):** peer → **body queue** (raw only) → **lookup** (in-order from `max(path_lo, lookup_taken_hi+1)`; decode + TipOnly `head_fk`; **dequeue** raw into `loadq=14`). Hole/densify/receive: in-hand = confirmed ∨ BQ hash ∨ `H ≤ lookup_taken_hi` (post-lookup reject except Cancelled and EngineFault leave taken_hi; SoftWire/Cascade/BadPrev rewind taken_hi to the confirmed tip; EngineFault stamp fail re-offers the **full** batch including the first height and does not `request_single_block`; epoch-stale loadq chunks re-offer on drop; BadPrev most-work rewind then sets the LCA) → **load** (recv load-sized batch; stamp binds carried BQ keys + BQ `header_fk` (skip header ensure/put when the store row matches) + pin + assemble) → scripts → write. Stage IO: [`invariants.md`](./invariants.md). **No** peer→confirm-feed wire retain. **No** hash-only / Class-A-only confirm (bq wire required). Load bind order is same-batch → in-flight → load-batch skeleton → Corrupt (plan=None leftover TipOnly). Lookup does not own a published identity chain. Class A sets loc once on the same `CreatePin` Arc load cloned into InFlight (`OnceLock`); later-wave stamp reads it (no disk loc-by-fk on that path). Confirm writes header size/weight as a sequential `header.body` rewrite of 96-byte records (`put_size_weight_run`, no insert `put_lock`); header-sync insert still stores zeros. Write drain inserts `tx.head` in parallel with Class C on the process-wide `ibd-confirm-head` thread. Drain complete is max inserted **fk** (not tip/fence — those advance during drain). Header-cache GC polls store tip every load pack. In-flight prune is **after the last load batch of a lookup wave finishes its in-flight read**, using the drain+fence height snapshotted before that wave's TipOnly (`docs/invariants.md`). No leftover pending map. Bodies without a known height are marked missing and re-getdata after the height map is ready — there is **no** dual-track archive-job / ContigPark fallback. Load pack **waits** on `feed.cv` when tip+1 is in `ready` but the BQ is not resolve-complete (no retain/BQ spin). Pack takes the feed mutex to collect candidates and again to mark inflight; one BQ `pack_snapshot` in between.
...
**Wire retained on the pipeline batch only:** lookup/load pull `bitcoin::Block` from the body queue; that wire rides through scripts; **no Class-A wire rebuild**. Script jobs carry lookup `TxPrecompute` (no job `from_tx`). Split Class A (`txout` / `seqsigwit` / `spent`) is planned once and committed in the write stage.
...
| Role | `tip-accept`, `ibd-confirm-*`, `rbtc-sh-wb`, `rbtc-bf-wb` | Sole writer for a resource. Not a job pool. |
...
| `rbtc-bf-wb` | **One** BIP158 basic filter appender (`--block-filter-index`), spawned when tip mode is entered. Waits on the same release gate as `rbtc-sh-wb` (`index_released_through`). Builds `(watermark, released tip]` from Class A outside its lock, then commits a chunk under the appender mutex only if the watermark and every `confirmed[h]` are unchanged; `disconnect_tip_with` takes that mutex before truncating. A gap wider than one 64-height chunk (the materialize, or later catch-up) runs up to 8 worker threads for the whole pass: they take chunks in order from a shared cursor (never more than 2 × workers ahead of the last commit), and this thread commits finished chunks in height order, up to 8 per durable put, checking stop between commits. Apply errors halt the node. IBD confirm does no filter work. |
...
| `MempoolHub::inner` | One `RwLock<ActiveMempool>`. **Write** is graph mutation + durable slot append only (microseconds at ~12k live). No Query / script verify / compact reconstruct under write. Prepare may hold a **read** across Query. Blocking `read`/`write` is `assert_not_reactor`. Session INV/getdata/compact use `try_read`; miss = skip that item (re-getdata / notfound / reconstruct without those txs). Compact fill siphashes live txid/wtxid and clones **matching** bodies only — never `list_live()`. Handshake `FeeFilter` / `-minrelaytxfee` is an **atomic**, not `inner`. Age-INV log and min live `accept_at` are **not** `inner` (same class as `accept_at`). Accept caches `{tip_header_fk, height, mtp}` until the header fk changes. `get_coin` computes create MTP only for BIP68 **time** locks and calls `block_tx_fks` only when input-0 is missing. `index_txid` hashes the admitted tx + prepare prevouts (no second Query). Do not CoW a 12k-txid presence set on every accept. Lock order: `inner` → `sh_index` / `unbroadcast` / `relay_*`. |
...
Cold parent `create.loc` / `txout.body` on the **load** thread uses
**FdOnly loc + bulk body** (`idx_body_pipeline` → `bulk_io` uring/pread). Batch
