...
| Load body outs | By `txout` range only from lookup stamp; incomplete outs → hard Err. Pin **copies** lookup `spent_range` (no loc IO). Later-wave InFlight loc comes from the CreatePin Arc (Class A `set_loc`); stamp disk loc-by-fk only for a true loc hole | **No** loc cold outs on load; **no** `inwit` on pin |
