...
| Ensure (write) | Every non-null spend edge has `spent_range` abs after ensure returns. Lookup stamped archived parents; same-batch from Class A append RAM (packed pin outs). Missing abs → `invariant:` | **No** write `create.loc` read |
