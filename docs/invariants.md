...
| Ensure (write) | Every non-null spend edge has `spent_range` abs after ensure returns. Lookup stamped archived parents; same-batch from append RAM loc; just-written from write-thread loc packs kept until write of the first height whose TipOnly had not started at note (`lookup_started_hi + 1`; `keep_until` never bumped). Missing abs → `invariant:` | **No** write `create.loc` read |
