...
| Ensure (write) | Every non-null spend edge has `spent_range` abs after ensure returns. Lookup stamped archived parents; same-batch from append RAM loc; just-written from write loc packs kept until write of `lookup_started_hi` at note (last TipOnly that may have missed disk loc). Missing abs → `invariant:` | **No** write `create.loc` read |
...
| `write_ensure_stamps_spent_range_after_load_pin` / `pin_and_ensure_journey` / `fill_same_batch_abs_from_append_loc_ram` | load pin copies lookup spent range; missing stamp is Corrupt; same-batch abs from append RAM; just-written loc until write of `lookup_started_hi` (no write loc pread) |
