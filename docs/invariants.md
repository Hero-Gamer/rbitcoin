...
| Ensure (write) | Every non-null spend edge has `spent_range` abs after ensure returns. Lookup stamped archived parents; same-batch from append RAM loc; just-written from write-thread loc packs kept until write of the last height whose TipOnly had started at note (`lookup_started_hi`; `keep_until` never bumped; fill of that write runs first). Later-wave InFlight identity still takes TipOnly loc. Missing abs → `invariant:` | **No** write `create.loc` read |
