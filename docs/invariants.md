...
| **Format migrate** | Occupied 22 rewrites `create.loc.ovf` 12 B→16 B; occupied 23 rewrites `header.body` 88 B→96 B; occupied 24 zero-extends `txstat.body`; `meta` to live `SCHEMA_VERSION`. Leftover fuse8 v1 / flat idx / Shared SH body / Paged pack8 **refuse** | Soft migrate (tmp+rename) or explicit wipe/rebuild |
