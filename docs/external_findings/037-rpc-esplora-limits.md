...
queue is HTTP 503. **0** is unlimited.
...
socket or when join-header trust is set. Loopback alone does not.
...
`rbitcoin-node` `config::tests::rpc_work_queue_defaults_finite_and_zero_is_unlimited`,
`rbitcoin-esplora` `server::client_id_tests::loopback_without_trust_ignores_client_header`.
