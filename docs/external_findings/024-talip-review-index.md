...
| H-3 | high | Witness padding caches block hash | fixed | `padded_coinbase_witness_over_weight_is_not_cached_invalid` ([034](./034-witness-padding-not-cached.md)) |
| #08 / #01 / #04 | critical | Header error discarded; chain wipe; work wrap | fixed | `zero_prev_with_live_tip_is_not_held` ([030](./030-header-accept.md)) |
...
| H-4 | high | Unbounded per-peer send queue | fixed | `getheaders_flood_stops_at_the_send_budget_and_getaddr_is_once` ([043](./043-peer-send-buffer.md)) |
...
| C06 / C04 | critical | Compact partial count, one partial, pending-header cap | fixed | `reconstruct_rejects_tx_count_above_weight_ratio`, `pending_header_insert_past_cap_clears` ([029](./029-compact-tx-count.md)) |
...
| M-7 | medium | Addr relay fanout | fixed | `addrv2_reaches_one_or_two_neighbors_and_stops_at_the_burst` ([045](./045-addr-relay.md)) |
