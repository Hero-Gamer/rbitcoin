...
| Check | Who |
|-------|-----|
| `fmt` `deny` `clippy` `ast-grep` `test` `windows` `macos` `coverage` `nixos-module-eval` `mutants (0/4)`–`(3/4)` | Every PR (`ci.yml`). A mutants shard killed at 30 minutes passes; a finished non-zero exit fails. |
