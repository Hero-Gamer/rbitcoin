...
| Mempool / fees | done | `/mempool`, `/mempool/txids`, `/mempool/txids/page[/:last]`, `/mempool/recent` (accept-order ring), `/fee-estimates` (sat/vB at the estimator's 1 sat/kvB resolution). mempool.space `/api/v1/fees/recommended` is its backend's surface, not served here (**404**) |
...
| `{ "action": "want", "data": ["stats"] }` | Immediate `{ "mempoolInfo", "fees" }` (`mempoolInfo` is the `GET /mempool` JSON; `fees` is mempool.space's recommended tiers in sat/vB at the estimator's resolution); re-push on announce/tip when fee-snapshot `computed_at` changes or 1 s age elapses. Combine with `blocks` |
